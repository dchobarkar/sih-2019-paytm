...
def send_test():
   ...
   # from functions.sqlquery import sql_query
    ...
    push_service = FCMNotification(
        api_key="AAAAYNJto2I:APA91bEHdYbm2mhUfGfBIWseFzintaOJgJDifgSyzJdfcZrCyUdLD9XPwSKlX6IMaOeXNMkdTSs52mGKVequsJ_JtdZcq-6e15L9UzjHhs2JaMhcHmpDPgBu0D0bmPyXO2pqHa2Q0q0d")
    ...
    # registration_id = "de_Gt1FMzSk:APA91bH2U7fXr4WVaSVHNzRY0iWFf09WTy5ZrZsBvrtUmi-sTT2ssLvI-GCO26ZbIZUGCY9pCwWScZRHoD3KKbG4MXT_lJLJm9di2Gjbc_2RhVY0Qlp22Keag4ZL4LMgwERPW6WuhZFD"
   ...
   # registration_id = "dZyJARo3dYo:APA91bHDbv5zwUwhKCzH_CrziVrujev_gkSzvvqjWdtUeN9j1evYXmHOM37zeT9I5IrQZY-3pNXznDWbxtnpmH0aUnEQRzTIxTJ6oLhXLRHynhDxEl-7Zr551jrB4w0449GAp62WDhmo"

    message_title = ["SIH 2019 - PayTM",
                     "Independence Day Sale", "Complete your Profile"]
    message_body = ["GecaToons", "Flat Rs.1000 off on every product",
                    "Please complete your profile"]

    # def job():
    result = push_service.notify_single_device(
        registration_id=registration_id, message_title=message_title[1], message_body=message_body[1])
...
def getRecom(id):
    ...
    final_answer = []
    results = sql_query2(
        ''' SELECT product_2_id FROM data_table WHERE product_1_id = ? and factor > 0.5''', (store,))
    # sirf dikhaneke liye
...
def push_Data():
    ...
    time_array = ['if_opted_in', 'if_zoomed_in',
                  'if_checked_details', 'if_checked_reviews']
    time_dict = {'if_opted_in': 2, 'if_zoomed_in': 2,
                 'if_checked_details': 3, 'if_checked_reviews': 3}
    total_time = 0
    ...
    results = sql_edit_insert(
        ''' INSERT INTO searches (user_id,search1_id, time_spent) VALUES (?,?,?)''', (user_id, search_id, total_time))
...
def getUserTime():
    ...
    from functions.sqlquery import sql_query2
    results = sql_query2(
        ''' INSERT INTO user_time (user_id, notif_category, time_id) VALUES (?,?,?)''', (user_id, notif_category, time_id))
    results2 = sql_query2(
        ''' SELECT * from user_time WHERE user_id = ? ''', (user_id,))
...
def mapUserTime():
    ...
    results = sql_query2(''' INSERT INTO notifications (user_id, product_id, time_of_sending, time_of_action,action) VALUES (?,?,?,?,?)''',
                         (user_id, product_id, time_of_sending, time_of_action, action))
    results2 = sql_query2(
    ...
    if action == 0:
        ...
        #from functions.sqlquery import sql_query2, sql_query
        results = sql_query2(
            ''' SELECT search1_id, time_spent from searches where user_id = ?''',  (user_id,))
        results = results[-10:]
        ...
        df2 = pd.read_csv('products.csv', usecols=[
                          'product_id', 'number_of_purchases'])[1:162]
...
def deleteData():
    ...
    results = sql_delete(
        ''' DELETE from searches where user_id = ?''', (user_id,))
    return jsonify(results)
...
def send_product_notification():
   ...
   # return "hora hai"
    push_service = FCMNotification(
        api_key="AAAAYNJto2I:APA91bEHdYbm2mhUfGfBIWseFzintaOJgJDifgSyzJdfcZrCyUdLD9XPwSKlX6IMaOeXNMkdTSs52mGKVequsJ_JtdZcq-6e15L9UzjHhs2JaMhcHmpDPgBu0D0bmPyXO2pqHa2Q0q0d")
    registration_id = "fKD3beCbh14:APA91bFMxIksQPtLSkhnrguzNADX9LRUNy5Gdu_r1p45I1O722gfXoFr8tq1PrhDoMWcfD7_8-rDQwPt4EiigOXIoc9kZQW7SBzwFptCosj4EZ5mJlirUwG3G0_lZVZdTnd_scVeTZoY"
    message_title = "We think you will love"
    message_body = best_product
    result = push_service.notify_single_device(
...
def getBest():
    ...
    dar_id = result_list[-1]

    time_id = str(get_time())
    return jsonify(time_id)

    from functions.sqlquery import sql_edit_insert
    return_value = sql_edit_insert(
        '''  INSERT INTO user_notifications (user_id, product_id, time_id) VALUES (?,?,?) ''', (user_id, dar_id, time_id))
...
def get_time():
    ...
    df1 = pd.read_csv('time_data.csv', usecols=[
                      'send_time',	'response_time', 'action', 'no_of_noti'	])
